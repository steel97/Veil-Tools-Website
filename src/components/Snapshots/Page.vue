      ...
      <div>
        ...
        <div v-else>
          ...
          <div class="flex flex-col lg:flex-row items-center lg:items-start mt-4">
            <div v-for="(snapshot, index) in bestMirror.snapshots" :key="`bsnapshot-${index}`" class="md:mr-1 grow">
              <div class="flex justify-between flex-col lg:flex-row">
                <div>
                  <a
                    :href="bestMirror.path + snapshot.name" class="underline text-blue-800"
              ...
              <div v-if="index === 0" class="flex items-center text-sm text-blue-800 my-2 md:mb-0">
        ...
        <div v-if="network !== null" class="flex flex-wrap justify-between lg:grid grid-cols-2 mx-auto">
          <div v-for="(mirror, index) in network.mirrors" :key="`cmirror-${index}`" class="text-sm mt-3">
            ...
            <div v-for="(snapshot, sindex) in mirror.snapshots" :key="`mirror-${index}-snapshot-${sindex}`">
              <a
                :href="mirror.path + snapshot.name" class="underline text-blue-800"
                rel="noopener noreferrer nofollow noindex"
              >{{ snapshot.name }}</a>
            </div>
          </div>
      ...
      new Promise((resolve) => {
        ...
        $fetch(
          ...
          measureResults.push({
            ...
            speed: Number.parseFloat(downloadSpeed),
          });
          resolve();
        }).catch(() => {
...
onMounted(() => {
  if (!config.public.snapshotsEnabled)
    return;
